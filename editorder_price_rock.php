<?php 
    ...
    $query = "SELECT * FROM `edit_order_rock` WHERE 1";
    $result = mysqli_query($conn, $query);

    $id=isset($_GET['id']) ? $_GET['id']:'';
    if($id!=''){
        $query = "SELECT * FROM `edit_order_rock` WHERE no='".$id."'";
        $result = mysqli_query($conn, $query);
 ...
 <div class="container">
	<div class="row">
		<div class="col-md-4 col-md-offset-4 well" style="font-family: 'Kanit', sans-serif; font-size: 16px;">
			<form method="post" id="form" enctype="multipart/form-data" action="action_editorder_rock.php?id=<?php echo $row["no"];?>"></a>
			<?php if($id!=''){?>
		...
		<!-- แก้ไขราคา -->
                    ...
                    <div class="form-group">
						...
						<input type="text" name="price" placeholder="" minlength="" required class="form-control" value="<?php echo $row['price'];?>" /> 
						<span class="text-danger"></span> 
					</div>	
