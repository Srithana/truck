 ...
 <div class="container">
	<div class="row">
		<div class="col-md-4 col-md-offset-4 well" style="font-family: 'Kanit', sans-serif; font-size: 16px;">
			<form method="post" id="form" enctype="multipart/form-data" action="action_editorder_tom.php?id=<?php echo $row["no"];?>"></a>
			<?php if($id!=''){?>
		...
		<!-- แก้ไขราคา -->
                     <div class="form-group">	
						...
						<label >: <?php echo $row['name11'] ?></label>
						<input type="hidden" name="name11" placeholder=""  class="form-control" value="<?php echo $row['name11'] ?>" />
						<span class="text-danger"></span>
					</div>
						
                    <div class="form-group">
						<label for="name">ราคา</label>
						<input type="text" name="tom_sand" placeholder="" minlength="" required class="form-control" value="<?php echo $row['tom_sand'];?>" /> 
						<span class="text-danger"></span> 
					</div>
